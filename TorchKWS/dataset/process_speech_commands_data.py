    ...
    if class_split == "sub":
        ...
        for file in os.listdir(folder):
            if ".wav" in file:
                ...
                total_iters = max(1, (len(y) - sampling_rate) // silence_stride)
                for i in tqdm(range(0, len(y) - sampling_rate, silence_stride), desc="Generating silence slices", total=total_iters):
...
def main():
    ...
    parser.add_argument("--data_root", required=False, default='/root/autodl-tmp/SpeechCommands', type=str)
    parser.add_argument('--data_version', required=False, default=2, type=int, choices=[1, 2])
    ...
    data_set = "gsc_v{0}_35".format(args.data_version)
