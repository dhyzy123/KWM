...
from networks.kwm.utils import LabelSmoothingLoss
...
def get_dataloader_keyword(data_path, class_list, class_encoding, parameters, noise_aug=False):
    ...
    valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size, num_workers=0, shuffle=False, drop_last=True)
...
class Trainer:
    ...
    def __init__(self, opt, model, total_class_num):
        self.opt = opt
        self.lr = opt.lr
        self.step = opt.step
        ...
        self.device = torch.device(f"cuda:{opt.gpu}")
        ...
        self.criterion = LabelSmoothingLoss(num_classes=self.total_class_num, smoothing=0.1)
        self.loss_name = {
    ...
    def model_save(self):
        save_directory = os.path.join("/root/autodl-tmp/Result/model_save", self.opt.save)
    ...
    def model_train(self, optimizer, scheduler, train_dataloader, valid_dataloader):
        ...
        for self.epo in range(self.epoch):
            ...
            for batch_idx, (waveform, labels) in tqdm(enumerate(train_dataloader), position=0, total=len(train_dataloader)):
                ...
                if scheduler["warmup"] is not None and self.epo < 10:
                    scheduler["warmup"].step()
                elif scheduler["scheduler"] is not None:
