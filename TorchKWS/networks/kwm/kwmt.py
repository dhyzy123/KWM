...
class Block(nn.Module):
    ...
    def forward(
        ...
        if not self.fused_add_norm:
            ...
            fused_add_norm_fn = (
                rms_norm_fn if isinstance(self.norm2, RMSNorm) else layer_norm_fn
            )
            if residual is None:
                hidden_states, residual = fused_add_norm_fn(
                    hidden_states,
...
class KWMT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        num_patches = int(img_size[0]/patch_size[0] * img_size[1]/patch_size[1])
        self.num_patches = num_patches
        ...
        # original init
        self.to_patch_embedding.apply(segm_init_weights)
...
def kwmt_from_name(model_name: str, num_classes: int, depth: int):
